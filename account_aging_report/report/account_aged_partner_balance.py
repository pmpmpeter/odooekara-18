...
class AgedPartnerBalanceCustomHandlerInherit(models.AbstractModel):
    ...
    def _report_custom_engine_aged_receivable_invoice_date(self, expressions, options, date_scope, current_groupby, next_groupby, offset=0, limit=None, warnings=None):
        return self._aged_partner_report_custom_engine_common_invoice_date(options, 'asset_receivable', current_groupby, next_groupby, offset=offset, limit=limit)

    def _report_custom_engine_aged_payable_invoice_date(self, expressions, options, date_scope, current_groupby, next_groupby, offset=0, limit=None, warnings=None):
        return self._aged_partner_report_custom_engine_common_invoice_date(options, 'liability_payable', current_groupby, next_groupby, offset=offset, limit=limit)

    def _aged_partner_report_custom_engine_common_invoice_date(self, options, internal_type, current_groupby, next_groupby, offset=0, limit=None):
        ...
        query = f"""
            ...
            JOIN period_table ON
                ...
                (
                    ...
                    OR COALESCE(account_move_line.date, account_move_line.date) >= DATE(period_table.date_stop)
                )
