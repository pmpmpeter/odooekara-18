...
class AccountMove(models.Model):
    ...
    payment_method_line_id = fields.Many2one('account.payment.method.line', string='Payment Method',
        ...
        compute='_compute_payment_method_line_id_move',
        domain="[('id', 'in', available_payment_method_line_ids)]")
    available_payment_method_line_ids = fields.Many2many('account.payment.method.line',
        compute='_compute_payment_method_line_fields_move')
    ...
    check_amount_in_words = fields.Char(
        ...
        store=True,
        compute='_compute_check_amount_in_words_move',
    )
    check_amount_in_num = fields.Float(
    ...
    def _compute_check_amount_in_words_move(self):
        # pdb.set_trace()
        # print("Case1111111111111111ffffffffffffffffffffffffffffffffffffffffffff")
        # for move in self.filtered(lambda l: l.journal_id in ['bank'] and l.move_type in ['entry']):
        for move in self:
            # print("Case111111111111111111111111111111")
            if move.check_amount_in_num>0:
                move.check_amount_in_words = move.journal_id.company_id.currency_id.amount_to_text(move.check_amount_in_num)
            else:
                for line in move.line_ids.filtered(lambda l: l.account_id.account_type in ['asset_cash']):
                    if line.debit >0:
                        move.check_amount_in_words = move.journal_id.company_id.currency_id.amount_to_text(line.debit)
                    elif line.credit >0:
                        move.check_amount_in_words = move.journal_id.company_id.currency_id.amount_to_text(line.credit)
                    else:
                        move.check_amount_in_words = False
                for line in move.line_ids.filtered(lambda l: l.account_id.name in ['Outstanding Receipts', 'Outstanding Payments']):
                    if line.debit >0:
                        move.check_amount_in_words = move.journal_id.company_id.currency_id.amount_to_text(line.debit)
                    elif line.credit >0:
                        move.check_amount_in_words = move.journal_id.company_id.currency_id.amount_to_text(line.credit)
    ...
    def _get_cash_bank_journal_type(self):
        for line in self:
            if line.journal_id:
                if line.journal_id.type in ['bank']:
                    line.is_bank_entry = True
                else:
                    line.is_bank_entry=False
            else:
                line.is_bank_entry=False

    def _get_payment_method_codes_to_exclude_move(self):
    ...
    def _compute_payment_method_line_fields_move(self):
        for pay in self:
            pay.available_payment_method_line_ids = pay.journal_id._get_available_payment_method_lines(pay.payment_type)
            to_exclude = pay._get_payment_method_codes_to_exclude_move()
            if to_exclude:
    ...
    def _compute_payment_method_line_id_move(self):
        for pay in self:
    ...
    def print_checks(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_cheque_amount': self.check_amount_in_num,
    ...
    def action_print_cheque_payment(self):
        if self.cheque_format_id:
            ...
            return self.env.ref('account.report_payment_account_move_cheque').report_action(self, data=data)
